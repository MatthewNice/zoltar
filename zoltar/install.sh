...
echo "=========================="
echo "Installing App zoltar"

# Here is where we perform installation of scripts, services, etc.
echo " - Installing ROS packages for zoltar..."
...
runuser -l $LIBPANDA_USER -c /etc/libpanda.d/apps/zoltar/installRosPackages.sh
...
rosrun robot_upstart install zoltar/launch/${LAUNCH_FILE} --user root

echo "Enabling can_to_ros startup script"
sudo systemctl daemon-reload
sudo systemctl enable zoltar
popd
echo "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
